;**************************************************************************************************
;*    C02BIOS 5.0 - Release version for Pocket SBC  (c)2013-2025 by Kevin E. Maier 10/04/2025     *
;*                                                                                                *
;* BIOS Version 5.0 supports the following 3.3V hardware specification:                           *
;*                                                                                                *
;*  - W65C02S with clock rate up to 8.0 MHz                                                       *
;*  - AS6C1008 128KB Static RAM mapped to 56KB or 62KB                                            *
;*  - SST39LF-010 128KB Flash ROM mapped to 2KB or 8KB                                            *
...
;                                    START OF BIOS CODE                                           *
;**************************************************************************************************
;C02BIOS version used here is 5.0 (new release with IDE bootable support)
;
...
;
                BBR5    MATCH,IDE_RD_ERR        ;Check for Drive Present, branch if not (5/6)
;
...
;
; For now, we just jump to the Monitor Cold Start vector. Once the Boot Record code is
; completed, we'll manage a user timeout feature to either default boot the IDE device
; or allow the user to interrupt the IDE boot and jump to the Monitor.
;
;                JMP     LBA_BUFFER      ;We're done, execute partition code
                JMP     M_COLD_MON      ;Safety for debug (3)
;
BAD_PART_BLK
...
;
                BRA     IDE_NO_BOOT     ;Enter Monitor Cold Start (3)
;
IDE_RD_ERR
...
IDE_NOT_FOUND
                ...
                LDY     #>NO_DRIVE_MSG  ; (2)
                JSR     PROMPTR         ;Send Message to console (6)
IDE_NO_BOOT     JMP     M_COLD_MON      ;Enter Monitor Cold Start (3)   
;**************************************************************************************************
...
; To enable a level of benchmarking, the following routines are part of C02BIOS version 5.x
;
...
; setup is completed. A routine does this is also called by the Reset-Diag function (JMP $FF00).
;
DETECT_IDE
                ...
                RTS                     ;Return to caller, no IDE controller found (6)
;
...
LBA_SIZE        LDY     LBA_OFFSET-1,X  ;Get Offset to LBA count (4)
                ...
                LDA     #$02            ;Command for set write cache (2)
;
...
IDE_RESET                               ;Do a Reset of IDE device
                ...
                LDA     #$02            ;Set A Reg to enable Write Cache (2)
                JSR     IDE_SET_CACHE   ;Enable Write Cache (6)
